...
/** Match as returned from API (dates may be ISO strings). */
type ApiMatch = {
  id: number;
  sport: string;
  homeTeam: string;
...
/** Match definition in seed data (optional times/scores). */
type SeedMatch = {
  id?: number;
  sport: string;
  homeTeam: string;
...
/** Single commentary/feed entry in seed data. */
type FeedEntry = {
  matchId?: number;
  message: string;
  minute?: number;
...
/** Per-match state used during seeding. */
type MatchState = {
  match: ApiMatch;
...
async function readJsonFile(fileUrl: URL | string): Promise<unknown> {
  const raw = await fs.readFile(fileUrl, "utf8");
  return JSON.parse(raw);
}

async function loadSeedData(): Promise<LoadedSeedData> {
  const parsed = (await readJsonFile(DEFAULT_DATA_FILE)) as Record<string, unknown> & {
    commentary?: unknown[];
    feed?: unknown[];
    matches?: SeedMatch[];
  };
  ...
  if (Array.isArray(parsed)) {
    return { feed: parsed as FeedEntry[], matches: [] };
  }

  if (Array.isArray(parsed.commentary)) {
    return { feed: parsed.commentary as FeedEntry[], matches: parsed.matches ?? [] };
  }

  if (Array.isArray(parsed.feed)) {
    return { feed: parsed.feed as FeedEntry[], matches: parsed.matches ?? [] };
  }
...
async function fetchMatches(limit = 100): Promise<ApiMatch[]> {
  ...
  return Array.isArray(payload.data) ? (payload.data as ApiMatch[]) : [];
}
...
async function createMatch(seedMatch: SeedMatch): Promise<ApiMatch> {
  ...
  const responsePayload = (await response.json()) as { match?: ApiMatch };
  const match = responsePayload.match;
  if (!match) {
    throw new Error("Create match response missing match.");
  }
  return match;
}
...
async function insertCommentary(
  matchId: number,
  entry: FeedEntry,
): Promise<{ id: number; matchId: number; message: string }> {
  ...
  const responsePayload = (await response.json()) as {
    commentary?: { id: number; matchId: number; message: string };
  };
  const commentary = responsePayload.commentary;
  if (!commentary) {
    throw new Error("Create commentary response missing commentary.");
  }
  return commentary;
}
...
function cricketBattingTeam(entry: FeedEntry, match: ApiMatch): string | null {
...
function normalizeCricketFeed(entries: FeedEntry[], match: ApiMatch): FeedEntry[] {
  const sorted = [...entries].sort((a, b) => {
    const inningsDiff = inningsRank(a.period) - inningsRank(b.period);
    if (inningsDiff !== 0) {
      return inningsDiff;
    }
    const seqA: number = Number.isFinite(a.sequence) ? (a.sequence as number) : Number.MAX_SAFE_INTEGER;
    const seqB: number = Number.isFinite(b.sequence) ? (b.sequence as number) : Number.MAX_SAFE_INTEGER;
    if (seqA !== seqB) {
      return seqA - seqB;
    }
    const minA: number = Number.isFinite(a.minute) ? (a.minute as number) : Number.MAX_SAFE_INTEGER;
    const minB: number = Number.isFinite(b.minute) ? (b.minute as number) : Number.MAX_SAFE_INTEGER;
    return minA - minB;
  });

  const grouped = new Map<number, FeedEntry[]>();
...
function cloneCommentaryEntries(
  ...
  return entries.map((entry) => {
    const next: FeedEntry = { ...entry, matchId: targetMatch.id };
    ...
    next.message = replaceTrailingTeam(entry.message ?? "", replacements) as string;
    return next;
  });
}
...
function expandFeedForMatches(
  ...
  const byMatchId = new Map<number, FeedEntry[]>();
  for (const entry of feed) {
    const mid = entry.matchId;
    if (mid === undefined || !Number.isInteger(mid)) {
      continue;
    }
  ...
  for (const match of seedMatches) {
    ...
    if (matchId !== undefined) {
      if (!templateBySport.has(match.sport) && byMatchId.has(matchId)) {
...
function buildRandomizedFeed(
  ...
  const buckets = new Map<number | null, FeedEntry[]>();
  for (const entry of feed) {
  ...
  const matchIds: number[] = Array.from(buckets.keys()).filter(
    (id): id is number => id !== null,
  );
  const randomized: FeedEntry[] = [];
  let lastMatchId: number | null = null;

  while (randomized.length < feed.length) {
    const candidates = matchIds.filter((id: number) => (buckets.get(id) ?? []).length > 0);
    if (candidates.length === 0) {
      break;
    }

    let selectable = candidates;
    if (lastMatchId !== null && lastMatchId !== undefined && candidates.length > 1) {
      const withoutLast = candidates.filter((id: number) => id !== lastMatchId);
...
function getMatchEntry(
  ...
  return entry.matchId !== undefined ? matchMap.get(entry.matchId) ?? null : null;
}
...
async function seed(): Promise<void> {
  ...
  if (Array.isArray(seedMatches) && seedMatches.length > 0) {
    for (const seedMatch of seedMatches) {
      ...
      const seedMatchId = seedMatch.id;
      if (seedMatchId !== undefined && Number.isInteger(seedMatchId)) {
        matchMap.set(seedMatchId, {
          match,
          score: { home: match.homeScore ?? 0, away: match.awayScore ?? 0 },
          fakeNext: Math.random() < 0.5 ? "home" : "away",
        });
  ...
  for (let i = 0; i < randomizedFeed.length; i += 1) {
    ...
    if (!target) {
      console.warn("⚠️  Skipping entry: matchId missing or not found:", entry.message);
      continue;
    }
